...
class hmm:
	...
	def forward_back(self):
		...
		for x in self.obs: # grabbed each line of the observed data
			...
			alpha 	   = np.zeros([T,K])				
			for j in range(1,K):
				alpha[0,j] = prior[j]+ emit[j,x[0]]
 
				
			for k in range(1,T):
				for j in range(K):
					additions = trans[:,j]+alpha[k-1]			
					m = np.max(additions)
					additions = additions - m
					alpha[k,j] =emit[j,x[k]]+m+np.log(sum(np.exp(additions)))
			beta = np.zeros([T,K])
			for k in range(T-2,-1,-1):
				for j in range(K):
					additions = emit[:,x[k+1]]+beta[k+1]+trans[j,:]
					m = np.max(additions)
					additions = additions -m
					beta[k,j]= m + np.log(sum(np.exp(additions)))
