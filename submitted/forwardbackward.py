...
class hmm:
	...
	def forwardAndbackward(self):
		...
		for x in self.obs: # grabbed each line of the observed data
			...
			alpha 	   = np.zeros((T,K))				
			beta  	   = np.zeros((T,K))		
			#print("b[:,x] {}".format(self.b[:,x[0]]))
			alpha[0,:] = np.multiply(self.b[:,x[0]], self.pr)  
			#print("alpha = {}".format(alpha))
			#print(" a trans = {}".format(self.a.T))
			for t in range(1,T):
				#print("t = ",t)
	...
	def postprocess(self,predicted_file,metric_file):
		...
		elements = 0
		count = 0	
		#print("tags = {}".format(self.state))
		#print("predicted = {}".format(self.predicted))
		for x, y in zip(self.state,self.predicted):
			...
			for i,j in zip(x,y):
				if i!=j : 
					count +=1
				elements +=1
		#print("elements = {}, count = {}".format(elements,count))
		accuracy = 1 - count/elements
		#print("Accuracy: ",accuracy)
		
		#print("{}".format(self.indexToTag))
		predicted  = []
		# now write the predicted in tags
